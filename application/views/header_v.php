<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
<!DOCTYPE html>
<html lang="ko">
	<head>
		<title>알렌카 - 세계 최고 효율의 금연법</title>
		<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=0">		
		<meta property="og:title" content="알렌카 - 세계 최고 효율의 금연법" />
		<meta property="og:type" content="website" />
		<meta property="og:url" content="http://www.allencarr.co.kr" />
		<meta property="og:image" content="http://www.allencarr.co.kr/images/img_allencarr_sns.jpg"/>

		<link rel="stylesheet" type="text/css" href="/include/animate.css">
		<link rel="stylesheet" type="text/css" href="/include/bootstrap.min.css">		
		<link rel="stylesheet" type="text/css" href="/include/ac-font-line.css">
		<link rel="stylesheet" type="text/css" href="/include/ac-blank.css">
		<link rel="stylesheet" type="text/css" href="/include/ac-color.css">
		<link rel="stylesheet" type="text/css" href="/include/ac-div.css">

		<script src="/include/jquery.min.js"></script>
	</head>
	<body class="pdg15 bwhitez1">

	
	<div style="background-color: white; height: 30px; width: calc(100% - 30px); position: fixed;  z-index: 255; top:-15px;"></div>
	<div style="background-color: #141414; height: 65px; width: calc(100% - 30px); position: fixed;  z-index: 255;">
		<div class="h025"></div>
		<div class="container-fluid pdglr30">		
			<p class="tari cp2 tsilver lt000" style="font-family: 'Arial';">
				<b style="padding-right: 25px;">SCHEDULE</b>
				<b style="padding-right: 25px;">VALUES</b>
				<b style="padding-right: 25px;">METHOD</b>
				<b style="padding-right: 25px;">TEAM</b>
				<b style="padding-right: 25px;">TESTIMONIAL</b>
				<b style="padding-right: 25px;">PARTNERS</b>
				<b style="padding-right: 25px;">FAQ</b>
			</p>
		</div>
	</div>
	<div class="h065"></div>	
	